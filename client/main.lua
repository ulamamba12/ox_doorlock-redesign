...
RegisterNetEvent('ox_doorlock:setState', function(id, state, source, data)
    ...
    if double then
        ...
        if door.holdOpen then
            DoorSystemSetHoldOpen(door.hash, door.state == 0)
        end
...
RegisterNetEvent('ox_doorlock:editDoorlock', function(id, data)
    ...
    if double then
            ...
            if data then
                if data.doorRate or door.doorRate or not data.auto then
                    DoorSystemSetAutomaticRate(
                        doorHash,
                        data.doorRate or door.doorRate and 0.0 or 10.0,
                        false,
                        false
                    )
                end
                ...
                if data.holdOpen then
                    DoorSystemSetHoldOpen(doorHash, doorState == 0)
                end
            else
        ...
        if data then
            ...
            if data.holdOpen then
                DoorSystemSetHoldOpen(door.hash, doorState == 0)
            end
        else
            DoorSystemSetDoorState(door.hash, 4, false, false)
...
local function requestDoorlockTexture(textureDict)
    ...
    while not HasStreamedTextureDictLoaded(textureDict) do
        if GetGameTimer() > timeout then
            print(('[ox_doorlock] Failed to load texture dict: %s'):format(textureDict))
            return false
        end
...
CreateThread(function()
    ...
    local showUI
    local textureDict = Config.DoorlockTextureDict or 'ui_doorlock'
    local lockedTexture = Config.DoorlockLockedTexture or 'ui_door_locked'
    local unlockedTexture = Config.DoorlockUnlockedTexture or 'ui_door_unlocked'
    local spriteWidth = Config.DoorlockSpriteWidth or 0.070
    local spriteHeight = Config.DoorlockSpriteHeight or 0.075
    local spriteOffsetZ = Config.DoorlockSpriteOffsetZ or 0.0

    local textureLoaded = requestDoorlockTexture(textureDict)
        ...
        if num > 0 then
            ...
            for i = 1, num do
                ...
                if door.distance < door.maxDistance then
                    ...
                    if textureLoaded and not door.hideUi then
                        local textureName = door.state == 1 and lockedTexture or unlockedTexture

                        SetDrawOrigin(
                            door.coords.x,
                            door.coords.y,
                            door.coords.z + spriteOffsetZ,
                            0
                        )
