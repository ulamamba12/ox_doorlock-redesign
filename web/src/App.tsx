...
const App: React.FC = () => {
  ...
  useNuiEvent('setVisible', (data?: number | boolean) => {
    if (data === false) {
      setVisible(false);
      navigate('/');
      return;
  ...
  useNuiEvent('updateDoorData', (data: DoorColumn | number) => {
    if (typeof data === 'number') {
      return setDoors(doors.filter((door) => door.id !== data));
    }
  ...
  return (
    <Box className={classes.container}>
      <Transition transition="slide-up" mounted={visible} duration={220} timingFunction="ease">
        {(style) => (
