...
import { ActionIcon, Box, Group, Modal, Select, Text, Tooltip } from '@mantine/core';
import { TbLockAccess, TbSettings, TbTrash } from 'react-icons/tb';
    ...
    <>
      <Box sx={{ width: '100%' }}>
        {lockpickFields.map((field, index) => {
          const value = typeof field === 'string' ? field || null : 'custom';
          const difficultyLabel = value
            ? selectData.find((item) => item.value === value)?.label || 'Custom'
            : 'Not selected';

          const canDelete = lockpickFields.length > 1;
              ...
              <Group position="apart" noWrap spacing={12}>
                <Group spacing={10} noWrap sx={{ flex: 1, minWidth: 0 }}>
                  ...
                  <Box sx={{ flex: 1, minWidth: 0 }}>
                    ...
                    >
                      Difficulty #{index + 1} - {difficultyLabel}
                    </Text>
